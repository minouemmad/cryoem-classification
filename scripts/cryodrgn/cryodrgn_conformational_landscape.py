...
# Figures
# --------------------------------------------------------------------------- #
def fig_landscape(pcs, hard, names, order, colors, evr, path_arrows, out):
    fig, ax = plt.subplots(figsize=(8.4, 7.0))
    ax.hexbin(pcs[:, 0], pcs[:, 1], gridsize=70, cmap="Greys", mincnt=1, alpha=0.55)
    for nm in order:
        i = names.index(nm)
        m = hard == i
        if m.sum() < 10:
        ...
        e = Ellipse(mu, w, h, angle=ang, facecolor=colors[i], edgecolor="k",
                    lw=1.0, alpha=0.28, zorder=2)
        ax.add_patch(e)
        ax.scatter(*mu, s=150, c=[colors[i]], edgecolors="k", marker="o",
                   zorder=4, label=nm)
    # hypothesised motion path between class means
    for a, b in path_arrows:
        if a in names and b in names:
            ...
            ax.annotate("", mb, ma, zorder=3,
                        arrowprops=dict(arrowstyle="-|>", color="0.25", lw=2,
                                        connectionstyle="arc3,rad=0.12"))
    ax.set_xlabel(f"PC1  ({evr[0]*100:.0f}% of latent variance)")
    ax.set_ylabel(f"PC2  ({evr[1]*100:.0f}%)")
    ax.set_title("Conformational landscape coloured by CryoSPARC class")
    ax.legend(loc="upper left", fontsize=8, ncol=2, title="class")
    fig.tight_layout(); fig.savefig(out); plt.close(fig)


def ridgeline(ax, coord, hard, names, order, colors, title, xlabel):
    order_use = [nm for nm in order if (hard == names.index(nm)).sum() >= 20]
    grid = np.linspace(np.percentile(coord, 0.5), np.percentile(coord, 99.5), 400)
    step = 0.85
    for row, nm in enumerate(order_use):
        i = names.index(nm)
        xi = coord[hard == i]
        d = gaussian_kde(xi)(grid); d = d / d.max()
        base = (len(order_use) - row) * step
        ax.fill_between(grid, base, base + d, color=colors[i], alpha=0.85, lw=0)
        ax.plot(grid, base + d, color="k", lw=0.6)
        ax.axvline(xi.mean(), ymin=0, ymax=1, color=colors[i], lw=0, alpha=0)
        ax.text(grid[0], base + 0.15, nm, fontsize=9, fontweight="bold", va="bottom")
    ax.set_yticks([])
    ax.set_xlabel(xlabel)
    ax.set_title(title)


def fig_conformational_axes(zstd, hard, names, order, colors, coords, out):
    fig, ax = plt.subplots(1, len(coords), figsize=(7.2 * len(coords), 6.4),
                           squeeze=False)
    for k, (label, frm, to) in enumerate(coords):
        axis = named_axis(zstd, hard, names, frm, to)
        coord = zstd @ axis
        ridgeline(ax[0][k], coord, hard, names, order, colors,
                  title=label, xlabel=f"{frm} \u2192 {to}  (latent projection)")
    fig.suptitle("Population density along conformational coordinates "
                 "(classes stacked in trajectory order)", fontweight="bold")
    fig.tight_layout(); fig.savefig(out); plt.close(fig)
...
def class_overlap(zstd, hard, names, order, colors, out, sample=40000, k=30, seed=0):
    ...
    for i in range(K):
        for j in range(i + 1, K):
            S = 0.5 * (covs[i] + covs[j]); dm = means[i] - means[j]
            Db = (0.125 * dm @ np.linalg.solve(S, dm)
                  + 0.5 * np.log(np.linalg.det(S)
                                 / np.sqrt(np.linalg.det(covs[i]) * np.linalg.det(covs[j]))))
            BC[i, j] = BC[j, i] = float(np.exp(-Db))
    # reorder rows/cols by trajectory order
    perm = [names.index(nm) for nm in order]
    onames = order
    Mo, BCo = M[np.ix_(perm, perm)], BC[np.ix_(perm, perm)]
    fig, ax = plt.subplots(1, 2, figsize=(13.5, 5.8))
    for a, Mx, ttl in ((ax[0], Mo, "k-NN neighbour-class fraction\n(off-diagonal = assignment overlap)"),
                       (ax[1], BCo, "Bhattacharyya coefficient\n(latent distribution overlap)")):
        im = a.imshow(Mx, cmap="magma", vmin=0, vmax=1)
        a.set_xticks(range(len(onames))); a.set_yticks(range(len(onames)))
        a.set_xticklabels(onames, rotation=45, ha="right", fontsize=8)
        a.set_yticklabels(onames, fontsize=8)
        a.set_title(ttl)
        for i in range(len(onames)):
            for j in range(len(onames)):
                a.text(j, i, f"{Mx[i,j]:.2f}", ha="center", va="center",
                       color="w" if Mx[i, j] < 0.55 else "k", fontsize=6.5)
    ...
    for i, nm in enumerate(names):
        row = M[i].copy(); row[i] = -1; j = int(np.argmax(row))
        confused[nm] = {"partner": names[j], "neighbour_frac": round(float(M[i, j]), 3),
                        "self_frac": round(float(M[i, i]), 3)}
    return confused


def fig_core_states(zstd, hard, names, order, colors, keep, coords, path_arrows, out):
    Zs, Hs = zstd[keep], hard[keep]
    pca = PCA(3).fit(Zs); P = pca.transform(Zs)
    core_order = [nm for nm in order if names.index(nm) in set(Hs.tolist())]
    fig, ax = plt.subplots(1, 2, figsize=(15, 6.4))
    # left: landscape
    ax[0].hexbin(P[:, 0], P[:, 1], gridsize=60, cmap="Greys", mincnt=1, alpha=0.55)
    for nm in core_order:
        i = names.index(nm); m = Hs == i
        mu = P[m, :2].mean(0); cov = np.cov(P[m, :2].T)
        vals, vecs = np.linalg.eigh(cov)
        ang = np.degrees(np.arctan2(vecs[1, np.argmax(vals)], vecs[0, np.argmax(vals)]))
        w, h = 2 * 1.5 * np.sqrt(np.maximum(vals, 1e-9))
        ax[0].add_patch(Ellipse(mu, w, h, angle=ang, facecolor=colors[i],
                                edgecolor="k", lw=1, alpha=0.30))
        ax[0].scatter(*mu, s=130, c=[colors[i]], edgecolors="k", zorder=4, label=nm)
        ax[0].annotate(nm, mu, fontsize=9, fontweight="bold", ha="center", va="center")
    for a, b in path_arrows:
        if a in core_order and b in core_order:
            ma = P[Hs == names.index(a), :2].mean(0)
            mb = P[Hs == names.index(b), :2].mean(0)
            ax[0].annotate("", mb, ma, arrowprops=dict(arrowstyle="-|>", color="0.25", lw=2,
                           connectionstyle="arc3,rad=0.12"))
    ax[0].set_xlabel("PC1'"); ax[0].set_ylabel("PC2'")
    ax[0].set_title("Core-state landscape (ablated classes removed)")
    ax[0].legend(fontsize=8, ncol=2)
    # right: ridgeline along the first coord using core-only projection
    label, frm, to = coords[0]
    axis = named_axis(Zs, Hs, names, frm, to)
    ridgeline(ax[1], Zs @ axis, Hs, names, core_order, colors,
              title=label, xlabel=f"{frm} \u2192 {to}")
    fig.suptitle("Core conformational states (SC..V-shaped)", fontweight="bold")
    fig.tight_layout(); fig.savefig(out); plt.close(fig)
...
def main():
    ...
    fig_landscape(pcs, hard, names, order, colors, evr, path_arrows,
                  os.path.join(args.outdir, "landscape_by_class.png"))
    fig_conformational_axes(zstd, hard, names, order, colors, coords,
                            os.path.join(args.outdir, "conformational_axes.png"))
    fe = fig_free_energy(pcs, hard, names, order, colors,
                         os.path.join(args.outdir, "free_energy.png"), args.barrier_kt)
    confused = class_overlap(zstd, hard, names, order, colors,
    ...
    if args.ablated:
        ...
        core, _, _ = fig_core_states(zstd, hard, names, order, colors, keep, coords,
    ...
    lines += ["", "## Most-confused class partner (k-NN latent neighbours)"]
    for nm, d in confused.items():
        lines.append(f"- {nm}: self {d['self_frac']}, nearest other = **{d['partner']}** ({d['neighbour_frac']})")
    if core:
