#!/usr/bin/env bash
###############################################################################
# FOCUSED cryoDRGN pipeline for J1442-homorefine (J4001) — hudson
#
# J1442_homorefine = fresh homogeneous refinement of the same 230,396 J1442/
# J1497 particles, single global-consensus alignments3D/pose (independent pose
# frame from the original J1442_real run). Mirrors focused_pipeline.sh /
# focused_pipeline_j399.sh. Same 4 steps:
...
#
# Prereq: run scripts/cryodrgn/j1442_homorefine_recover_states.sh FIRST (the
# "sweet spot" zdim16/beta0.03/D128 run) so there's an analyze.<EP>/kmeans20/
# to build the mask from. Default RUN below points at that run's output.
#
# USAGE (repo root on hudson, cryodrgn env):
#   export IMAGES_DIR=/path/to/parent/of/J995   # same images as the original J1442
#   bash scripts/cryodrgn/j1442_homorefine_recover_states.sh        # step 0: produce RUN
#   bash scripts/cryodrgn/focused_pipeline_j1442_homorefine.sh      # steps 1-4 below
###############################################################################
set -euo pipefail
...
RUN="${RUN:-$J1442_DIR/train_recover_D128_z16_b0p03}"   # from j1442_homorefine_recover_states.sh
EP="${EP:-50}"
APIX="${APIX:-2.075}"                                # J1442: orig box320 @0.83A -> D128 (same as original run)
...
PARTICLES="$RESID" TAG="train_focus_D128_z16_b0p03" \
  ...
  bash "$REPO/scripts/cryodrgn/j1442_homorefine_recover_states.sh"
...
reproducible before committing GPU to refinement.  Compare against the
original J1442_real focused run (results_cryodrgn/J1442_gP25_WT_POSE_BIAS/train_focus_*)
to check the states are the SAME across two independently-refined pose frames.
EOF
