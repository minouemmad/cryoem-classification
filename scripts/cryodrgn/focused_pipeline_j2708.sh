#!/usr/bin/env bash
###############################################################################
# FOCUSED cryoDRGN pipeline for J2708 (eP30, D1212 ATP/VX) — hudson
# Mirrors focused_pipeline.sh (built/validated for J1442). Same 4 steps:
...
#
# Prereq: run scripts/cryodrgn/j2708_recover_states.sh FIRST (the "sweet spot"
...
#   bash scripts/cryodrgn/j2708_recover_states.sh          # step 0: produce RUN
#   bash scripts/cryodrgn/focused_pipeline_j2708.sh         # steps 1-4 below
###############################################################################
set -euo pipefail
...
RUN="${RUN:-$J2708_DIR/train_recover_D128_z16_b0p03}"   # from j2708_recover_states.sh
EP="${EP:-50}"
APIX="${APIX:-2.0996}"                               # J2708: orig box250 @1.075A -> D128
VOLS="${VOLS:-$RUN/analyze.$EP/kmeans20/vol_*.mrc}"

PARTICLES="${PARTICLES:-$J2708_DIR/inputs/particles.128.mrcs}"
POSES="${POSES:-$J2708_DIR/inputs/poses.pkl}"
CTF="${CTF:-$J2708_DIR/inputs/ctf.pkl}"

FOCUS_DIR="${FOCUS_DIR:-$J2708_DIR/focus}"
RESID="${RESID:-$J2708_DIR/inputs/particles.128.focus.mrcs}"
...
PARTICLES="$RESID" TAG="train_focus_D128_z16_b0p03" \
  ...
  bash "$REPO/scripts/cryodrgn/j2708_recover_states.sh"

echo "== 4. score the focused latent (did the mobile region split?) =="
"$PY" "$REPO/scripts/cryodrgn/cryodrgn_sweep_score.py" \
  --runs "$J2708_DIR/train_focus_D128_z16_b0p03" \
  -o "$J2708_DIR/train_focus_D128_z16_b0p03/focus_score" --sep-thresh 1.5
