#!/usr/bin/env bash
###############################################################################
# J264 "high-recall" state-recovery run with AUTO-RESUME & VALIDATION
# Mirrors j1442_recover_states.sh for the J264 (E1371Q / ATP) dataset.
...
# Base dir for J264 cryoDRGN artifacts
J264_DIR="${J264_DIR:-$REPO/results_cryodrgn/J264_real}"

# Inputs (particles.128.mrcs already exists -> downsample is skipped)
PASS="${PASS:-$REPO/data/J264/cryosparc_P7_J264_passthrough_particles_all_classes_blob.cs}"
POSES="${POSES:-$J264_DIR/inputs/poses.pkl}"
CTF="${CTF:-$J264_DIR/inputs/ctf.pkl}"
...
OUT="$J264_DIR/$TAG"
PARTICLES="${PARTICLES:-$J264_DIR/inputs/particles.${D}.mrcs}"
CLUSTERS="$OUT/cluster_exports_k${KCLUST}"
mkdir -p "$OUT"

echo "=============================================================="
echo " J264 high-recall run with AUTO-RESUME & VALIDATION"
