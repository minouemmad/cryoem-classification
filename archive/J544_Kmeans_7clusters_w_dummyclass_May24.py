...
N_DUMMIES = min(5, max(0, num_classes - 2))
N_KEEP = num_classes - N_DUMMIES
...
kmeans = KMeans(n_clusters = N_KEEP, init = mean_matrix[N_DUMMIES:,N_DUMMIES:], random_state = 0).fit(probabilities_kmeansarray[:,N_DUMMIES:])
kmean_centers = kmeans.cluster_centers_
kmeans_cov = np.cov(probabilities[:,N_DUMMIES:], rowvar = False)


# predicting which particles go where
kmeans_prediction = kmeans.predict(probabilities_kmeansarray[:,N_DUMMIES:])
kmeans_prediction = kmeans_prediction + (N_DUMMIES + 1)
...
pc_2d = pca_kmeans_2d.fit_transform(plotX.drop(columns = ['Original Assignment','K-means Assignment']))
pc_3d = pca_kmeans_3d.fit_transform(plotX.drop(columns = ['Original Assignment','K-means Assignment']))
...
plt.plot(range(1, num_classes + 1), np.cumsum(pca.explained_variance_ratio_), marker='o', linestyle='--')
...
# covariance heatmap
labels = [i for i in range(N_DUMMIES + 1, num_classes + 1)]
...
sns.heatmap(cm, annot=True, annot_kws={"fontsize": 8}, fmt="d", cmap="viridis", xticklabels = [i for i in range(1, num_classes + 1)], yticklabels = [i for i in range(1, num_classes + 1)])
...
for i in range(N_DUMMIES + 1, num_classes + 1):
...
cryos_counts = probabilities_df[probabilities_df['Original Assignment']>=N_DUMMIES + 1]['Original Assignment'].value_counts().sort_index()
